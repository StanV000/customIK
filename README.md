...
An educational C++ project for exploring robot kinematics and inverse kinematics (IK) from first principles. The goal is to build a small, understandable arm model that can compute forward kinematics, estimate Jacobians, and solve for joint angles that reach a target pose.

This repository is intentionally simple and hand-rolled. It is a good fit for learning how robotics math works in practice without depending on a large external library.
...
This project is mainly for learning and experimentation. It is not intended to be a production-ready robotics library, but it provides a clear and compact way to understand the fundamentals of kinematics and IK.

## Future ideas

- visualize the arm in 2D or 3D
- add more realistic joint limits and collision checks
- connect the solver to a ROS 2 or simulation workflow
- compare this approach with analytic IK for simple arms
